...
//#define USE_SBUS_RX
//#define USE_DSM_RX
static const uint8_t num_DSM_channels = 0b10; //If using DSM RX, change this to match the number of transmitter channels you have
...
//*
//This two sections below specifies the sensitivity of the IMU reading
//Default is the smallest range but most precise
//You might want higher range for more aggressive flight
...
//*
//The failsafe values are highly recommended defaults
...
float i_limit = 25.0;     //Integrator saturation level, mostly for safety (default 25.0)
                          //The max values below are the maximum angle or angular rates that can be commanded depending the controller type used in the void loop
...
void setup() {
  Serial.begin(500000); //USB serial, establish serial connection to onboard USB
  delay(500);
  
  //*
  //Initialize all pins
  //Motor and servo pins are assigned/attached to their designated pins from the pin declaration section
  ...
  //Initialize communication with desired radio type using function within radioComm sketch
  radioSetup();
  
  //*
  //Set radio channels to default (safe) values before entering main loop
  //Failsafe setup
  //Set radio channel variables to default values before entering main loop
  ...
  //Initialize communication with desired IMU
  IMUinit();
  ...
  //Get error offset from IMU gyro and accelerometer to apply to readings in the main loop
  //calculate_IMU_error(); //Calibration parameters printed to serial monitor. Paste these in the user specified variables section, then comment this out forever.

  //*
  //Arm servo channels
  //Write low value to servos/motors using PWM
  ...
  //Write low value to ESCs using OneShot125
  ...
  //*
  //ACTIONS NEEDED
  //Not included originally, modified
  ...
  //3 quick blinks to onboard LED to indicate main loop is starting
